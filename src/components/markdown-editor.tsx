...
export const MarkdownEditor = forwardRef<MarkdownEditorRef, MarkdownEditorProps>(
    ...
    if (bare) {
      base.push(
        EditorView.theme({
          ...
          "&.cm-focused .cm-selectionBackground": { backgroundColor: "color-mix(in srgb, var(--ms-primary) 34%, transparent)" },
  ...
  const editor = (
    <CodeMirror
      ...
      basicSetup={
        bare
          ? { lineNumbers: false, foldGutter: false, highlightActiveLine: false, highlightActiveLineGutter: false }
          : { lineNumbers: true }
      }
      className="h-full [&_.cm-editor]:h-full [&_.cm-scroller]:overflow-auto [&_.cm-editor]:bg-transparent"
    />
  );
